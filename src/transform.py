...
input_path = BASE_DIR / "data/raw/MICRODADOS_ENEM_2023.csv"
output_path = BASE_DIR / "data/processed/dados_processados.csv"


#%%
def atribuir_valores(df: pd.DataFrame) -> pd.DataFrame:
...
def definir_faixas_renda(df: pd.DataFrame) -> pd.DataFrame:
...
def convert_64_to_32(df: pd.DataFrame) -> pd.DataFrame:
...
def definir_estrutura_final(df: pd.DataFrame) -> pd.DataFrame:
    print("Definindo estrutura final de colunas...\n")

    df = df.rename(columns={"Q025" : "possui_internet"})

    df["possui_internet"] = np.where(df["possui_internet"]==2, True, False)
...
definir_faixas_renda(df)

convert_64_to_32(df=df)

calcular_scores(df)

definir_estrutura_final(df)
